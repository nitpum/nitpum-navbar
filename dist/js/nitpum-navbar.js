...
(function() {
      ...
      for (var j = 0; j < navbar[i].childNodes.length; j++) {
        if (navbar[i].childNodes[j].className == "nav-content") {
          ...
          for (var k = 0; k < navbar[i].childNodes[j].childNodes.length; k++) {
            if (
              navbar[i].childNodes[j].childNodes[k].className == "nav-burger"
            ) {
              (function(i, j, k) {
                navbar[i].childNodes[j].childNodes[k].addEventListener(
                  "click",
                  function() {
                    if (navbar[i].childNodes[j].className === "nav-content") {
  ...
  window.addEventListener("scroll", function() {
    ...
    for (
      var i = 0;
      i <
      document.getElementsByClassName("navbar-fixed-when-scroll-pass").length;
      i++
    ) {
      var fixedScrollingNav = document.getElementsByClassName(
        "navbar-fixed-when-scroll-pass"
      )[i];
    ...
    for (
      var i = 0;
      i < document.getElementsByClassName("navbar-hide-scrolling").length;
      i++
    ) {
      var hideScrollingNav = document.getElementsByClassName(
        "navbar-hide-scrolling"
      )[i];
